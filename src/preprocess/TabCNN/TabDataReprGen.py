###########################################################################################################################
# NOTE: Code was taken from TabCNN GitHub repo: https://github.com/andywiggins/tab-cnn/blob/master/data/TabDataReprGen.py #
###########################################################################################################################

# NOTE: Be careful about running this, some of the paths are hard coded and may not work on your machine
...
class TabDataReprGen:
    ...
    def get_nth_filename(self, n):
        ...
        filenames = list(filter(lambda x: x.endswith(".jams"), filenames))  # Converted to list for Python 3
...
def main(args):
    n = int(args[0])  # assuming the first argument is the script name and the second is the 'n' value
    m = args[1]  # assuming the mode is passed as the third argument
